...
const MASTER_PACKAGE_JSON_URL = 'https://raw.githubusercontent.com/andr-ll/cryl/master/package.json';
...
https.get(MASTER_PACKAGE_JSON_URL, async (res) => {
  ...
  res.on('end', () => {
    try {
      ...
      const updatedVersionArr = updatedVersion.split('.').map(numb => Number(numb));
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (updatedValue !== masterValue) {
          if (updatedValue < masterValue) {
            throw new Error(`The ${semanticVersion} version must not be less than ${masterValue}.`);
          }

          if (updatedValue !== masterValue + 1) {
            throw new Error(`The ${semanticVersion} version must be equal to ${masterValue + 1}.`);
          } else {
            for (let j = i + 1; j < 3; j++) {
              if (updatedVersionArr[j] !== 0) {
                throw new Error(`The ${semanticVersions[j]} version should be equal to 0 after updating ${semanticVersions[i]} version.`)
              }
            }
